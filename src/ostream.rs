...
impl<'a, F: Flush, H: Handoff<'a>> OStream<'a, F, H> {
    ...
    fn write_id_type(&mut self, id: Id, wire_type: u8) -> Result<()> {
        // `ID_MAX` is width-aware: the header is `(id << 3) | type` accumulated
        // in `Unsigned`, so a `value64`-off build cannot carry an id whose top
        // three bits fall off the value type. Reject it here rather than let `<<`
        // silently drop them and report success (documentation §5.1.2).
        if id > ID_MAX {
            return Err(Error::Argument);
        }
